...
namespace avocado {
   ...
   bool renderapp::on_init()
   {
      ...
      layout.add_attribute(0, vertex_layout::ATTRIBUTE_FORMAT_FLOAT, 3, false);
      layout.add_attribute(1, vertex_layout::ATTRIBUTE_FORMAT_FLOAT, 3, false);
      layout.add_attribute(2, vertex_layout::ATTRIBUTE_FORMAT_FLOAT, 2, false);
      ...
      wallLeft =    glm::translate(identity, glm::vec3(-50.0f, 50.0f, 0.0f) + center) *  glm::rotate(identity, -pi / 2.0f, zAxis) *   bgScale;
      wallRight =   glm::translate(identity, glm::vec3(50.0f, 50.0f, 0.0f) + center) *   glm::rotate(identity, pi / 2.0f, zAxis) *  bgScale;
      wallFront =   glm::translate(identity, glm::vec3(0.0f, 50.0f, 50.0f) + center) *   glm::rotate(identity, -pi / 2.0f, xAxis) *  bgScale;
      ceiling =     glm::translate(identity, glm::vec3(0.0f, 100.0f, 0.0f) + center) *   glm::rotate(identity, pi, zAxis) *   bgScale;
      

      if (!sampler.create(SAMPLER_FILTER_MODE_NEAREST, SAMPLER_ADDRESS_MODE_CLAMP, SAMPLER_ADDRESS_MODE_CLAMP)) {
   ...
   bool renderapp::on_tick(const time &deltatime)
   {
      ...
      //if (cam.position_.x > 45.0f) cam.position_.x = 45.0f;
      //if (cam.position_.x < -45.0f) cam.position_.x = -45.0f;
      //if (cam.position_.y > 45.0f) cam.position_.y = 45.0f;
      //if (cam.position_.y < 3.0f) cam.position_.y = 3.0f;
      //if (cam.position_.z > 45.0f) cam.position_.z = 45.0f;
      //if (cam.position_.z < -45.0f) cam.position_.z = -45.0f;
      
      {//crate
          const auto r = glm::rotate(identity, time::now().as_seconds(), glm::vec3(0.0f, 1.0f, 0.0f));
          const auto t = glm::translate(identity, glm::vec3(0.0f, 6.0f + 2.0f * sinf(time::now().as_seconds()), -10.0f));
          crate = centerMatrix * t * r;
      }
      {//metal
          metalBox = glm::translate(identity, glm::vec3(5.0f, 0.0f, 0.0f)) * crate;
      }
      {//earth
          earth = glm::translate(identity, glm::vec3(-5.0f, 0.0f, 0.0f)) * crate;
      }
   ...
   void renderapp::on_draw()
   {
      ...
      rend.set_sampler_state(sampler);
      ...
      {// walls & ceiling
          dynamic_array<glm::mat4> matrices = { wallLeft, wallRight, wallFront, wallBack, ceiling };
